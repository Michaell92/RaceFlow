...
- Create `src/modules/Race/tests/RaceEngine.spec.ts`
  ...
  - generateSchedule creates 6 rounds with correct distances and 10 horses each
...
- Create `src/modules/Race/stores/raceStore.ts`
  ...
  - Getters: `currentRound`, `currentRoundConfig`, `isLastRound`, `completedResults`
...
- Create `src/modules/Race/composables/useRaceAnimation.ts`
  - Input: `entries: RaceEntry[]`, `distance: number`
  - Output: `horsePositions: Ref<Map<number, number>>` (horseId → progress 0-100%), `isRunning`, `start()`, `pause()`, `resume()`
  - Uses `requestAnimationFrame` loop
  - Speed per horse = `baseFactor × (condition / 100) × randomFactor(0.85–1.15)` — recalculated at intervals for natural variation
  - Emits/calls a callback when first 10 horses cross finish → returns ordered results
  - Scales base race duration with distance (longer = proportionally longer animation)
...
- Smooth fade-in for results as rounds complete and display results in a modal
...
- Create `cypress/e2e/horseRacing.cy.ts`
  ...
  - Can start next round → runs through all 6 rounds
  - Generate Program resets and creates new schedule

### Step 18 — Documentation (_parallel with step 17_)

- `CLAUDE.md` — AI trace file documenting:
